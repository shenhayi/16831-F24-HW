...
class FFModel(nn.Module, BaseModel):
    ...
    def update(self, observations, actions, next_observations, data_statistics):
        ...
        obs_normalized = (obs - ptu.from_numpy(data_statistics['obs_mean'])) / ptu.from_numpy(data_statistics['obs_std'])
        acs_normalized = (acs - ptu.from_numpy(data_statistics['acs_mean'])) / ptu.from_numpy(data_statistics['acs_std'])

        delta_normalized = (delta - ptu.from_numpy(data_statistics['delta_mean'])) / ptu.from_numpy(data_statistics['delta_std'])
        ...
        loss = self.loss(delta_pred_normalized, delta_normalized)
        # Hint: `self(...)` returns a tuple, but you only need to use one of the
        # outputs.
